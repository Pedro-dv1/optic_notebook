...
export function PublicHeader({ mode = 'customer' }: { mode?: HeaderMode }) {
  ...
  const accountTo = user?.is_superuser ? '/platform' : user?.company ? '/admin' : '/cliente/agendamentos'
  ...
  return <header className="public-header sticky top-0 z-40 backdrop-blur-md">
    <div className="mx-auto flex h-20 max-w-[90rem] items-center justify-between gap-3 px-5 sm:h-[5.5rem] sm:gap-6 sm:px-8 lg:px-12">
      ...
      <nav className="hidden items-center gap-9 text-[1.02rem] font-medium text-[#6173a5] md:flex" aria-label="Navegação pública">
        ...
        {user ? <><Link to={isCustomer ? '/cliente/agendamentos' : accountTo}>{isCustomer ? 'Meus agendamentos' : 'Painel'}</Link>{isCustomer ? <AccountMenu /> : <Link to={accountTo} className="header-account-link" aria-label="Abrir minha conta"><ProfileAvatar src={user.avatar} name={user.full_name} className="size-full" /></Link>}</> : mode === 'home' ? <>
    ...
    {open && <nav className="absolute inset-x-0 top-full border-y border-[#d9e6f8] bg-white px-6 py-5 text-[#071044] shadow-sm md:hidden" aria-label="Navegação móvel">
      <div className="mx-auto grid max-w-lg gap-2"><a href="/#como-funciona" className="py-2" onClick={() => setOpen(false)}>Como funciona</a>
        {user ? isCustomer ? <><Link to="/cliente/agendamentos" className="py-2" onClick={() => setOpen(false)}>Meus agendamentos</Link><Link to="/cliente" className="py-2" onClick={() => setOpen(false)}>Buscar empresas</Link></> : <Link to={accountTo} className="py-2" onClick={() => setOpen(false)}>Painel</Link> : mode === 'home' ? <div className="flex items-center gap-4 py-2 text-[#6173a5]"><span aria-hidden="true">|</span><span>Contato</span><span>Políticas</span></div> : <><Link to={loginTo} className="py-2" onClick={() => setOpen(false)}>Entrar</Link><Link to={registerTo} className="public-primary-link mt-1 text-center" onClick={() => setOpen(false)}>Criar conta</Link></>}
      </div>
    </nav>}
  </header>
}
